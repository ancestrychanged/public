...
def process(
    ...
    logging.info("target wxh is %dx%d @ %d fps", w, h, fps)
    ...
    try:
        while True:
            ...
            f = cv2.resize(frame, (w, h), interpolation=cv2.INTER_AREA)
    ...
    logging.info("chunks made: %d", chunknumber if chunkSizes else chunknumber - 1)
...
def argParser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--fps", type=int, default=fps, help="target fps, defaults to %(default)s)")
    p.add_argument("--size", nargs=2, type=int, metavar=("WIDTH", "HEIGHT"), default=(w, h), help="target res, defaults to  %(default)s)")
    p.add_argument("--fpc", type=int, default=fpc, help="frames per chunk (default: %(default)s)")
    return p
